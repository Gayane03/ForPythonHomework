...
from lec_12.utils import execution_time
from lec_13.utils import generate_large_text_file


def count_words_chunk(chunk, word_freq):
...
def process_file_chunk(start, end, filename):
    with open(filename, 'r') as file:
        ...
        word_freq = {}
        count_words_chunk(text, word_freq)
        return word_freq
...
def count_words_simple(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            count_words_chunk(line, word_freq)
    return word_freq
...
def count_words_multithreading(filename, num_threads=10):
    ...
    with open(filename, 'r') as file:
        ...
        for i in range(0, len(file_content), chunk_size):
            ...
            thread = threading.Thread(target=count_words_chunk, args=(chunk, word_freq))
